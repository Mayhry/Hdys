		...
		$.get("js/index_clothes.json",function(data){
			for(var i=0;i<data.length;i++){
				if(data[i].id == proid){
					...
					var imgs = data[i].detailimg?data[i].detailimg:[];
					var sizes = data[i].size?data[i].size:[];
					var colors = data[i].color?data[i].color:[];
					...
					$(".old del").html(data[i].oldprice);
					$(".new em").text("￥"+data[i].newprice);
					if(data[i].salecount){
